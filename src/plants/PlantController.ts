...
class PlantController extends Controller{
  ...
  public async getPalmPlants(@Request() req: any): Promise<any>{
    ...
    const sql = `
    with PlantData as (
      select B.ASSET_ID, B.TRACKING_DT, B.COVERPLANTS, B.CROWNING, B.NOTES, B.PLANT_STATUS, B.SOIL, B.STEM_COUNT,
...
@Post('/upsertPlantdata')
  ...
  (@Request() req: any, @Header('x-user-id') userId: string, @Body() body: IupsertPlantdata): Promise<any>{
    ...
    const { ASSET_ID, TRACKING_DT, COVERPLANTS, CROWNING, NOTES, PLANT_STATUS,
      SOIL, STEM_COUNT, CANOPY_WIDTH, DIAMETER, HEIGHT, GRADE, FERTILIZER } = body;

    const sql = `
    insert into PLANT_DATA (ASSET_ID, TRACKING_DT, COVERPLANTS, CROWNING, NOTES, PLANT_STATUS, SOIL, STEM_COUNT, CANOPY_WIDTH, DIAMETER, 
    ...
    with inputdata as (
      ...
      nvl(:NOTES, '') NOTES, nvl(:PLANT_STATUS, '') PLANT_STATUS, nvl(:SOIL, '') SOIL, nvl(:STEM_COUNT, null) STEM_COUNT,
    ...
    currentData as (
      SELECT a.ASSET_ID, a.TRACKING_DT, a.COVERPLANTS, a.CROWNING, a.NOTES, a.PLANT_STATUS, a.SOIL, a.STEM_COUNT, a.CANOPY_WIDTH, a.DIAMETER, a.HEIGHT, a.GRADE, a.FERTILIZER
      FROM PLANT_DATA a
    ...
    SELECT 
    a.ASSET_ID ASSET_ID, nvl(nvl(a.TRACKING_DT, b.TRACKING_DT), null) TRACKING_DT, nvl(nvl(a.COVERPLANTS, b.COVERPLANTS), '') COVERPLANTS, nvl(nvl(a.CROWNING, b.CROWNING), '') CROWNING, 
    nvl(nvl(a.NOTES, b.NOTES), '') NOTES, nvl(nvl(a.PLANT_STATUS, b.PLANT_STATUS), '') PLANT_STATUS, nvl(nvl(a.SOIL, b.SOIL), '') SOIL, 
    nvl(nvl(a.STEM_COUNT, b.STEM_COUNT), null) STEM_COUNT, nvl(nvl(a.CANOPY_WIDTH, b.CANOPY_WIDTH), null) CANOPY_WIDTH, 
    nvl(nvl(a.DIAMETER, b.DIAMETER), null) DIAMETER, nvl(nvl(a.HEIGHT, b.HEIGHT), null) HEIGHT, nvl(nvl(a.GRADE, b.GRADE), '') GRADE, 
    nvl(nvl(a.FERTILIZER, b.FERTILIZER), '') FERTILIZER, trunc(SYSDATE) EFFDT, nvl((select max(EFFSEQ)+1 from PLANT_DATA
  ...
  `;
    ...
    const values = { ASSET_ID, TRACKING_DT, COVERPLANTS, CROWNING, NOTES, PLANT_STATUS, SOIL, STEM_COUNT,
